...
A local portfolio rebalancer with one Claude Code/Codex conversation as its only application command interface and a **view-only pie chart**, with no header, footer or other sections. A small lower-side ETH gas label shows the observed native balance outside allocation slices, weights and portfolio value. The [UI simplification](docs/prompts/015-pie-only-ui.md) keeps ticker/percentage labels within the chart; empty-wallet targets are explicitly distinguished from current holdings. Ask the agent to change AAPL from 20% to 30%; deterministic code updates the complete allocation and handles subsequent rebalancing. A repository skill starts and controls the local monitor; no embedded chat is needed.
