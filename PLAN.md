...
The initial [hook trust diagnosis](docs/prompts/021-hook-trust-handoff.md) found the hook untrusted. Later read-only discovery through both the standalone CLI and Desktop's bundled engine confirms it trusted and enabled. The [entry diagnostic](docs/prompts/023-desktop-dispatch-observation.md) subsequently recorded a real native prompt rejected as non-bare, with the runner still unarmed. The [browser-context correction](docs/prompts/024-browser-context-launch.md) supports the exact observed context framing around an otherwise bare request; metadata never supplies launch authority. Native hook entry is established, while Desktop-to-launcher dispatch remains unverified. Neither discovery nor isolated fixture tests establishes a live launch.
