...
Keep one pending operation at a time. Its pre-dispatch marker and subsequent transaction/provider identifier allow restart reconciliation. If a send may have succeeded but its identifier was not recorded, stop new execution with unresolved status rather than sending again. On Ledger rejection, keep monitoring but dismiss the signing request until a new connection or explicit agent request; do not reopen it every poll. The persisted cadence limits cycle frequency; it is not a gas budget or a one-transaction-per-hour guarantee. The [recovery stage](docs/RECOVERY.md) automatically handles stale raw-key sends after a fixed five-minute grace, with one zero-value cancellation at the same nonce. It retains both identities until a verified winning receipt; it never blindly retries an uncertain cancellation. A confirmed cancellation or original revert can continue the current active window. Canonical successful swaps are marked before pending is cleared: cycles with a successful swap retain hourly timing, while new cycles with no successful swap may retry after their original ten-minute window. Legacy records remain conservative. It does not invoke an LLM, stop/start itself or introduce a generic escalation framework. The explicit recovery command remains available.
