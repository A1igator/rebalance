...
A local portfolio rebalancer with one Claude Code/Codex conversation as its only application command interface and a **view-only pie chart**, with no header, footer or other sections. Actual holdings form the outer ring; a thin inner ring shows saved targets with matching colors and target percentages beside existing labels. The lower-left ETH gas balance includes an approximate USD value, with the current gwei rate and approximate swap/approval/rebalance costs below. ETH stays outside allocation slices, weights and portfolio value. The [UI simplification](docs/prompts/015-pie-only-ui.md), [target/gas display refinement](docs/prompts/030-gas-usd-and-target-ring.md) and [transaction-cost refinement](docs/prompts/031-transaction-fee-display.md) preserve explicitly labeled target-only empty wallets. Gas display combines cached public Robinhood/Coinbase quotes with measured application gas references and a bounded fixed-price projection through the pure planner. It never executes that projection or feeds it into trading. Ask the agent to change AAPL from 20% to 30%; deterministic code updates the complete allocation and handles subsequent rebalancing. A repository skill starts and controls the local monitor; no embedded chat is needed.
...
Keep one pending operation at a time. Its pre-dispatch marker and subsequent transaction/provider identifier allow restart reconciliation. If a send may have succeeded but its identifier was not recorded, stop new execution with unresolved status rather than sending again. On Ledger rejection, keep monitoring but dismiss the signing request until a new connection or explicit agent request; do not reopen it every poll. The persisted cadence limits cycle frequency; it is not a gas budget or a one-transaction-per-hour guarantee. The [recovery stage](docs/RECOVERY.md) automatically handles stale raw-key sends after a fixed 30-second grace, with one zero-value cancellation at the same nonce. It retains both identities until a verified winning receipt; it never blindly retries an uncertain cancellation. A confirmed cancellation or original revert can continue the current active window. Canonical successful swaps are marked before pending is cleared: cycles with a successful swap retain hourly timing, while new cycles with no successful swap may retry after their original ten-minute window. Legacy records remain conservative. It does not invoke an LLM, stop/start itself or introduce a generic escalation framework. Full raw-key launch can enter this automatic-recovery graph through unresolved/reverted state after successful preflight, preserving pending/cycle records; no separate user recovery or stop/start sequence is required. Setup-only and deferred signers do not enter this path. The explicit recovery command remains optional. See [the startup and gas-label correction](docs/prompts/029-automatic-launch-recovery-and-gas-label.md).
