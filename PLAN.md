...
A local portfolio rebalancer with one Claude Code/Codex conversation as its only application command interface and a **view-only pie chart**, with no header, footer or other sections. Actual holdings form the outer ring; a thin inner ring shows saved targets with matching colors and target percentages beside existing labels. The lower-left ETH gas balance includes an approximate USD value, with the current gas rate below in gwei and USD per gas unit. ETH stays outside allocation slices, weights and portfolio value. The [UI simplification](docs/prompts/015-pie-only-ui.md) and [target/gas display refinement](docs/prompts/030-gas-usd-and-target-ring.md) preserve explicitly labeled target-only empty wallets. Gas display uses cached public Robinhood RPC and Coinbase ETH-USD reference quotes, independently from trading. Ask the agent to change AAPL from 20% to 30%; deterministic code updates the complete allocation and handles subsequent rebalancing. A repository skill starts and controls the local monitor; no embedded chat is needed.
