...
For an uncertain send, read-only `recover` assesses public evidence. The explicit user command **`$rebalance recover`** can cancel the original nonce with a zero-value self-transfer and conditionally resume the previously active runner after verification. It spends gas, preserves both transaction identities and the existing cadence, and never blindly resends a swap. See [recovery behavior](docs/RECOVERY.md).
...
The optional [notification channel](docs/NOTIFICATIONS.md) feeds retained events into the **same running Claude session**. With `/rc`, that session can be used from a phone. Ledger drift, runtime-attention and completed-rebalance alerts are distinct; completion requires a confirmed swap and a fresh portfolio within the drift threshold. Phone pushes are Claude-controlled and require user setup. The channel neither signs nor relays permissions. Trading remains independent of it.
