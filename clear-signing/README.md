...
A separate [matching-context experiment](emulator/README.md) expands the fixed four-swap/five-account-call fixture into indexed fields and prepends a signed, firmware-enforced array-count constraint. It uses an explicit custom binary-protocol compiler extension because the stock ERC-7730 converter does not emit the required count guard. The original JSON descriptors and ordinary converter output remain unchanged. Exact-count emulator success is separate from signed-variant selection for other batch lengths and production EIP-7702 lookup.
