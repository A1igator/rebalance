# Original array-descriptor compatibility limit

The original array-based descriptors cannot complete a multi-call review through the pinned Ethereum app **1.22.3** and **erc7730 1.0.10** converter. This is a specific descriptor/toolchain limitation, not a conclusion about every possible Ledger integration or future release.
...
## Matching-context follow-up

The owner asked whether each nested call could receive matching context. A separate development experiment expands the exact synthetic array into indexed fields, retaining each call's target, native value, spender/caller and complete inner display. A custom binary-protocol compiler adds a signed RAW MUST_BE constraint on the actual ABI array length before those fields. This avoids accepting an extra undisplayed call; it does not remove the count constraint or alter the firmware. See [the current emulator evidence](../README.md) and [prompt108](../../../docs/prompts/108-matching-clear-signing-context.md).

This is a supported low-level firmware path with an explicit local compiler extension, not a claim that the stock ERC-7730 converter handles repeated scalar array context. Fixed four-swap/five-account-call variants do not establish flexible production descriptor selection. The original failed trace above remains valid history, while the indexed experiment provides a separate compatibility path. Production metadata trust and real delegated-account resolution remain unverified.
