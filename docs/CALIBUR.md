...
Current status under [prompt 097](prompts/097-simple7702-ledger-batching.md): the owner observed Ledger refuse Calibur delegation because the delegate was not whitelisted. The current undelegated-wallet Start path uses [canonical Simple7702Account](SIMPLE7702.md). This document retains the Calibur implementation, verified bytecode and earlier setup design; actual Calibur delegation or pending records keep their own path. No live Calibur activation receipt is claimed.
...
On first use, Ledger must approve a Robinhood-specific EIP-7702 authorization and the outer type-4 transaction. For a self-funded transaction at nonce N, its authorization uses N+1. Subsequent batches on the correctly delegated account need one outer transaction signature, although device UI can contain several review screens. The owner subsequently observed a physical whitelist refusal for Calibur authorization; successful activation and Calibur-specific Clear Signing remain unverified. A successful local test is not evidence of a device display or mainnet transaction.
...
This implementation was verified with isolated fixtures. The subsequent physical whitelist refusal prevented live activation; no live Calibur receipt is claimed here. Existing Calibur records remain supported without converting them to Simple7702.
