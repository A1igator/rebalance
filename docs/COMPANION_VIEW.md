...
Each chart listener keeps at most four long-lived status/view/setup streams, ending the oldest when another is admitted. Deliberate rotation sends an explicit event: status and view clients close that stream, perform bounded authoritative short reads, and wait 15 seconds before reconnecting. The Stop control keeps its latest observation during those reads, avoiding a false Unavailable flash; failed reads and actual stream errors still invalidate controls. Short `/api/view` reads and streamed view events share the full public snapshot contract, including portfolio routing. This leaves capacity for documents and short requests without disconnecting the chat or changing trading. See [prompt 099](prompts/099-funded-batch-planning-and-stream-rotation.md).
