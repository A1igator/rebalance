...
Targets and settings can be saved while running, including during receipt and signing waits. A known newer configuration triggers an immediate serial re-evaluation. Explicit target, allocation, shared strategy and Ledger rebalance requests bypass the automatic cooldown once, after pending reconciliation; ordinary settings edits retain recorded timing. Consumed request identities survive restart. See [live settings](LIVE_SETTINGS.md).

The scheduler retains one execution lock and runs one graph traversal at a time. Incoming activity is coalesced, not queued as thousands of future trades. After a receipt confirms, that same traversal refreshes holdings and can dispatch the next required approval/swap if the active window still allows it. Stops and pending barriers remain enforced before sending. Automatic timers and chain events do not extend the ten-minute active window or change successful-swap cadence. A fresh explicit rebalance request starts its own bounded cycle when work is dispatched; a no-trade request is consumed without creating an artificial cooldown.
