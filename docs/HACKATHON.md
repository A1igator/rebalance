...
- [x] Actual Ledger-backed approval protecting the transition from agent proposal to spending authority; [combined rebalance receipts verified](LEDGER_EXECUTION.md#verified-combined-rebalance--september-13).
...
The MVP prioritizes current Agent Stack/DMK and suitable swap primitives. Retain a focused Ring-backed credential demo for prize value, with actual allowed/denied-operation evidence. Demonstrate disconnected drift tracking followed by a fresh rebalance prompt on connection and actual device confirmation. No session delegation or spending-cap feature is part of the plan. Research-only findings and installed skills are not a substitute for device-backed functionality.
