...
| Ledger application | Nano Gen5 indexed account onboarding; direct DMK/Signer Kit integration; a physically confirmed **1.1875 USDG approval**, successful at block **61423563**. | A completed Ledger swap/rebalance, observed device wording, explicit rejection behavior and a reliable recording. The attempted Robinhood swap requested blind-sign fallback and the app refused it. |
...
The latest Ledger execution and feedback records establish approval signing; older historical notes about unverified transactions must be read at their recorded dates. An approval is real transaction evidence, but it is not a successful swap or portfolio completion.
...
5. **Decide the application path from evidence.** If the standalone test succeeds, document what it proves and reassess the exact Robinhood router/multicall display gap. Do not infer a network migration or replace the application flow. If it fails, capture the bounded failure stage and stop speculative retries; the existing verified approval and fallback refusal remain useful, accurately limited evidence.
...
Acceptance: a Ledger test is complete only when the record states its account, chain, exact operation, observed device outcome and independent transaction result or confirmed absence of submission. The preferred app milestone is one successful device-confirmed swap plus fresh holdings; approval-only, standalone CLI success and fixture tests remain separately labelled outcomes.
...
**2. Close the Ledger demonstration gap.** Validate [batched application rebalances](BATCHED_REBALANCES.md) to reduce the cash-funded four-stock case to one aggregate approval and one swap transaction. Public account discovery alone does not resolve the existing device display support gap. Pursue a scoped owner-controlled test once its actual blockers are resolved. The human-approval Ledger direction already uses meaningful official SDK primitives. Adding wallet-cli for its name or building Ring now is lower priority than a convincing transaction, refusal and outcome demonstration. CLI/Ring are necessary only for claims about the specific CLI/Ring directions; neither is a blanket requirement for this chosen SDK human-approval direction. Clear Signing remains an objective to verify, not an invented eligibility condition or an achieved feature.
...
| 1:25–2:25 | Ledger app/device interaction, observed outcome and receipt or explicit refusal state. | Explain that the backend prepares and the owner physically approves each transaction. Prefer a completed app swap; if only approval is verified, say so and show the unsupported swap refusal plainly. Any Ethereum CLI clip is labelled standalone compatibility testing. |
