...
| Ledger application | Nano Gen5 indexed onboarding; direct DMK/Signer Kit integration; a completed combined rebalance at **05:24 UTC September 13**: two exact approvals and one four-leg multicall, final receipt block **61705868**, followed by on-target app status. | A reliable recording, observed device wording and explicit rejection behavior. Earlier fallback failures are historical; success does not prove Clear Signing. |
...
The latest [Ledger execution record](LEDGER_EXECUTION.md#verified-combined-rebalance--september-13) establishes successful combined sales and purchases followed by portfolio completion. Show the [three-transaction public record](evidence/ledger-combined-rebalance-2026-09-13.json) for that run. September 12 sequential receipts and the earlier September 13 phase-batch audit remain dated history. Transaction count does not establish the number of physical review screens or taps.
...
5. **Decide the application path from evidence.** If the standalone test succeeds, document what it proves and reassess the exact Robinhood router/multicall display gap. Do not infer a network migration or replace the application flow. If it fails, capture the bounded failure stage and stop speculative retries; the verified combined application rebalance remains the primary demo evidence.
...
Acceptance: a Ledger test is complete only when the record states its account, chain, exact operation, observed device outcome and independent transaction result or confirmed absence of submission. The combined app milestone of one four-leg swap transaction plus fresh on-target holdings is established, with two distinct approvals in that run. Historical sequential/phase-batch results, approval-only attempts, standalone CLI checks and fixtures remain separately labelled outcomes.
...
**2. Record the verified combined Ledger flow.** The latest run completed with two distinct approvals and one sales-and-purchases multicall. [Batched application rebalances](BATCHED_REBALANCES.md) reduce a cash-funded four-stock case to one aggregate approval and one swap transaction; mixed holdings use one sales-and-purchases multicall with distinct approvals as needed. Use the verified run and its receipt in the recording; any additional owner-controlled test needs its own labelled outcome. Public account discovery alone does not resolve the separate device display support gap. The human-approval Ledger direction already uses meaningful official SDK primitives. Adding wallet-cli for its name or building Ring now is lower priority than a convincing transaction, refusal and outcome demonstration. CLI/Ring are necessary only for claims about the specific CLI/Ring directions; neither is a blanket requirement for this chosen SDK human-approval direction. Clear Signing remains an objective to verify, not an invented eligibility condition or an achieved feature.
...
| 1:25–2:25 | Ledger app/device interaction, observed outcome and receipt or explicit refusal state. | Explain that the backend prepares and the owner physically approves each transaction. Show the verified combined multicall and on-target result; explain the two distinct approvals plus one swap transaction in that run without equating transactions with review-screen count. Any Ethereum CLI clip is labelled standalone compatibility testing. |
