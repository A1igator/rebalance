...
After that, type **`$rebalance`** or select its project skill suggestion in the existing conversation. That is the launch request, including automatic real-money trading for raw-key/Privy or public monitoring for Ledger under the saved configuration; no separate application CLI command or arming message is part of normal operation. Trusting a hook alone does not submit that launch request. Native hook review is the user action the assistant cannot complete on the user's behalf.
...
Pending transactions and cycle timing are preserved. After successful read-only preflight, a full raw-key or Privy launch starts the ordinary runner even when a prior transaction is unresolved/reverted. Its graph reconciles and automatically recovers before any further trade; the launcher never calls the manual cancellation wrapper. Full Ledger launch may start receipt monitoring through unresolved/reverted state, but creates no signing request and never signs a cancellation. Setup-only stays unarmed, signer selection never falls back to raw keys, and malformed/failed checks or public errors still block. A newer stop issued during setup wins over the older launch request; the final conditional start compares the saved stop identity under the same short control lock used by `stop`. A changed configuration after preflight blocks that launch. No spending cap, budget accounting or LLM trading decision is introduced. Raw-key/Privy keep automatic signing; Ledger keeps physical confirmation for every transaction. See [the startup correction](prompts/029-automatic-launch-recovery-and-gas-label.md).

Possible results distinguish `armed`, setup-only `ready`, `starting`, `busy`, `needs-input`, `blocked` and an `already-handled` request. Inspect the accompanying public state and messages. An armed process can still be waiting for a receipt/cooldown or reporting an error; for Ledger it means monitoring is active, not permission to sign. If a start may have occurred but status cannot be reread, the result explicitly says state is unknown; it does not claim trading is unarmed. No launch result proves a mined swap or phone delivery.
...
## Pasted strategy import

The exact whole native user request `rebalance:v1 ...` takes a separate configuration-import path before startup routing. Codex's existing UserPromptSubmit script calls `share receive`; Claude adds a dedicated import-only UserPromptSubmit script while retaining its single launch UserPromptExpansion handler. Thus the earlier “no UserPromptSubmit launch” restriction remains intact. OpenCode checks one native user text part plus root Build/message identity before its ordinary command/notification path. A stable native request applies once to the attached wallet, or opens the selector unapplied. No pasted text gains runner-launch authority. See [sharing](SHARING.md), [prompt 087](prompts/087-deterministic-share.md) and its [automatic-application clarification](prompts/088-apply-pasted-strategy.md).
