...
Initial research: **2026-09-04**. The September 4 findings below preceded installation or device work. The [September 7 adoption record](#deterministic-account-onboarding--2026-09-07) adds a tested setup implementation and exact SDK dependencies; physical address verification and a sequential Ledger rebalance subsequently succeeded. See [the current execution evidence](LEDGER_EXECUTION.md#verified-sequential-rebalance-batch-validation-pending); live batching and readable display remain separate checks. Read the [minimal-scope decisions](prompts/006-minimal-mvp.md) and [latest direct-signing decision](prompts/008-direct-signing-and-ledger-connect.md) alongside the current plan.
...
## Dedicated Uniswap provider check — September 12

Under [prompt 080](prompts/080-ledger-uniswap-provider-check.md), the owner requested an actual service/CLI check after comparing Rebalance's direct Router02 signing with the dedicated Wallet CLI Uniswap path. [Sanitized evidence](evidence/ledger-uniswap-provider-check-2026-09-12.json) records the observed responses; no signing or device command was run.

The current inspected development source is [Ledger Live a9ce5a0](https://github.com/LedgerHQ/ledger-live/tree/a9ce5a0bbcc9d2a94cb6de9936c7ff556ad14127), whose Wallet CLI package says **2.5.0**. npm instead reported **2.1.0** as the latest published CLI and returned E404 for 2.5.0. The existing official 2.1.0 binary was used with a clean environment and disposable `XDG_STATE_HOME`. Its Segment analytics has no documented opt-out; automatic approval review initially blocked execution. The owner explicitly approved the read-only CLI check with that analytics disclosure before the binary was run. No embedded credential was used outside the unmodified CLI.

Observed results:

- Official `assets token robinhood ...` succeeded for canonical USDG and AAPL, returning their chain-specific IDs, matching contract addresses and correct decimals. Initial direct catalog requests had returned identical Cloudflare 1010/HTTP 403 responses for all five Robinhood assets and an Ethereum USDT control. Those direct failures therefore were not token-absence evidence.
- The independent [`/v5/currencies/all`](https://github.com/LedgerHQ/ledger-live/blob/a9ce5a0bbcc9d2a94cb6de9936c7ff556ad14127/libs/ledger-live-common/src/exchange/swap/api/v5/fetchCurrencyAll.ts) request with `providers-whitelist=uniswap` and `additional-coins-flag=false` returned 416 Robinhood IDs in each direction, including all five exact-address portfolio IDs. Catalog presence alone does not establish a provider route; the result also contains other networks.
- The published CLI's `swap quote` rejected the real Robinhood USDG ID with its Bitcoin/Ethereum/Solana restriction. The first attempt lacked required account flags; the corrected probe used visibly unconfigured labels, and verified source order plus the observed error establish rejection before account/session resolution. No session or account was fabricated. The [newer quote command](https://github.com/LedgerHQ/ledger-live/blob/a9ce5a0bbcc9d2a94cb6de9936c7ff556ad14127/apps/wallet-cli/src/commands/swap/quote.ts) retains the same guard. This is a quote-command limitation, not proof that every EVM execution route is unsupported.
- Read-only service requests for **1.1875 USDG** into each of AAPL/NVDA/MSFT/AMD, using actual catalog IDs and the selected public wallet, each returned HTTP 403 Cloudflare HTML. No valid quote/customFields was obtained, so `/v5/uniswap/swap` construction and device display could not be checked faithfully. No quote, permit signature or token ID was invented. Full query URLs and Cloudflare HTML are excluded from committed evidence.

The dedicated path remains worth investigating once those gates are resolved. Its [builder](https://github.com/LedgerHQ/ledger-live/blob/a9ce5a0bbcc9d2a94cb6de9936c7ff556ad14127/libs/ledger-live-common/src/wallet-api/Exchange/dex/dexDataBuilders.ts) uses provider-built calldata and the Uniswap device app; the [shared signer](https://github.com/LedgerHQ/ledger-live/blob/a9ce5a0bbcc9d2a94cb6de9936c7ff556ad14127/libs/ledger-live-common/src/wallet-api/Exchange/intents/shared/signTransactionEvm.ts) still handles blind-signing fallback. Neither the CLI's advertised swap command nor catalog recognition proves readable fields for Robinhood. Rebalance's app, running services, transaction encoding and fallback refusal were unchanged.

Unsent support question: Can Ledger enable the published CLI's quote path for Robinhood mainnet 4663, explain the credential-free Uniswap quote endpoint's Cloudflare rejection, and confirm the Uniswap device app can clear-sign USDG-to-stock swaps for these catalog IDs? Please identify required firmware/app versions, trusted token/contract context and a supported quote/build route; recognition in the catalog is already verified.
