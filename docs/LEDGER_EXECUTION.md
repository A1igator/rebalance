...
## Verified approval and remaining swap gap — September 12
...
The following swap ended `unsupported` at `signer.eth.steps.blindSignTransactionFallback`; no swap transaction was recorded. The owner reported a device message asking to enable transaction signing in Settings and then asked for a fix preserving readable signing. Blind-signing fallback remains refused. No new attempt is warranted merely because Retry is now available; the unsupported display path is unchanged.
