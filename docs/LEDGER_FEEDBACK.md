...
The owner's subsequent request in [prompt 083](prompts/083-atomic-sales-and-purchases.md) combines those two phases into one existing-router multicall. Sales send USDG to the wallet; purchases consume only starting cash plus enforced minimum proceeds, preserving a rounded reserve. This still uses Uniswap's official chain-4663 SwapRouter02 and Ledger DMK/Signer Kit. It does not introduce a separate Ledger swap router or use the standalone CLI provider. Full mixed calldata and receipt progression are covered by offline real-builder/runtime fixtures; a new physical combined-batch result remains unverified.
