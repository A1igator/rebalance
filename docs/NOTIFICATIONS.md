...
The shared `src/notification-filter.ts` sits before both chat transports, independently of trading. It recognizes only the fixed portfolio-observation read-failure event; unknown failures, configuration/transaction-state errors, unresolved/reverted transactions, Ledger requests and completed rebalances retain immediate delivery. Raw events remain in `events.json`; a separate ignored notification record tracks incidents and suppressed IDs, without deleting events or inventing acknowledgements. The CLI still exposes unacknowledged raw history for inspection.

A brief read failure that resolves within two minutes produces no chat wakeup. A continuing failure has one representative alert, surviving acknowledgement and listener restart. After an alert, distinct fresh successful observations spanning one minute reset the incident quietly. Intermediate graph writes with `error: null`, retained snapshots and repeated reads of the same timestamp do not establish stable recovery. Successful automatic transaction recovery also stays quiet, as requested. Notification filtering never runs recovery or changes execution controls.

Queue/status file events and exact incident deadlines drive eligibility; there is no periodic model check or healthy timer sweep. A filter error retains routine events and permits unrelated critical events to pass. A currently running trading process can continue emitting its original events: reloading only the notification worker applies the filter to them. Already accepted native prompts cannot be recalled by this filter. An existing Claude channel loads the update when it reconnects; no live Claude reload or phone delivery is implied by code tests.
