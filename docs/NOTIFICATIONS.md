...
The shared `src/notification-filter.ts` sits before both chat transports, independently of trading. It recognizes the exact fixed hashless portfolio-observation and swap-quote failure events. Each has a separate persistence journal; unknown failures, configuration/transaction-state errors, unresolved/reverted transactions, Ledger requests and completed rebalances retain immediate delivery. Raw events remain in `events.json`; a separate ignored notification record tracks incidents and suppressed IDs, without deleting events or inventing acknowledgements. The CLI still exposes unacknowledged raw history for inspection.

A brief recognized read or quote failure that resolves within two minutes produces no chat wakeup. Quote bootstrap starts its persistence window from newly observed failure evidence, so an old event cannot lend its age to a fresh outage. A current matching error also needs a recent status-file write; an explicitly stopped runner cannot revive a routine alert by rewriting retained state. These checks retain history without claiming recovery. A continuing failure has one representative alert, surviving acknowledgement and listener restart. After an alert, distinct fresh successful observations spanning one minute reset the incident quietly. Intermediate graph writes with `error: null`, retained snapshots and repeated reads of the same timestamp do not establish stable recovery. A newer holdings timestamp alone cannot clear a quote failure: quote recovery requires a fresh completed traversal that passed quote or has no trade to perform. Successful automatic transaction recovery also stays quiet, as requested. Notification filtering never runs recovery or changes execution controls.

Queue/status file events and exact incident deadlines drive eligibility; there is no periodic model check or healthy timer sweep. A filter error retains routine events and permits unrelated critical events to pass. A currently running trading process can continue emitting its original events: reloading only the notification worker applies the filter to them. Both transports recheck current eligibility immediately before sending a serialized event. A veto does not mark the event sent, so a later meaningful failure remains deliverable. Codex additionally attempts to withdraw only accepted native queue IDs recorded by Rebalance when their corresponding retry alerts become durably obsolete. Native withdrawal uses the supported `thread/queue/delete` RPC over a short-lived `codex app-server --listen stdio://` connection, without reading/resuming/starting a task or listing other queued prompts. A prepared withdrawal and every result retain the no-resend barrier. Deletion is best-effort: Desktop may already have fetched the prompt, so it is not proof that a message never started. Receiver instructions request silent acknowledgement of a stale alert that still reaches the agent, without a recovery, acknowledgement or no-action announcement; higher-priority host instructions still apply. An existing Claude channel loads the update when it reconnects; no live Claude reload or phone delivery is implied by code tests.

## Claude Code

The optional project `.mcp.json` starts `src/channel.ts` over stdio. It uses the official MCP SDK and Claude's channel extension, with one `acknowledge_event` tool. It exposes no HTTP endpoint, trading tool, key, permission relay or alternative chat interface. Commands still use the project skill and CLI. A notification may cause Claude to respond; that inference is outside the deterministic trading graph. The shared `src/event-stream.ts` watches parent-directory replacements of `events.json` and `status.json`, with an exact timer only while a read incident awaits a decision. It replays immediately after initialization, coalesces file hints and delivers serially. A healthy idle channel has no periodic queue sweep. Read, watcher or delivery failures alone create bounded retries (one second growing to thirty seconds). Watcher reconnection replays changes missed while disconnected. A stalled transport ends its notification subprocess after ten seconds with durable events retained; it never launches a concurrent resend.
...
Our worker has **no healthy queue-sweep timer**: startup replay and queue/status file changes drive it. Exact incident deadlines handle read-alert eligibility; bounded transport retries occur only after actual failures. Inside the installed Codex runtime, a **ten-second shared-queue revision check** discovers additions made by another process. This is deterministic native infrastructure, not a scheduled LLM prompt/check, but it means the complete Desktop path is not literally event-only. Normal idle conversations then wake; active turns finish first. An interrupted conversation deliberately retains a paused queue, and this worker never overrides that interruption. Busy turns, host availability, I/O and model reporting can add delay; ten seconds is not a delivery guarantee.
