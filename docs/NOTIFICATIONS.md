...
The [deterministic launch hook](LAUNCH.md) is a separate user-command entry point. It recognizes the exact supported bare launch forms and the separate explicit `$rebalance recover` operation; scheduled notification prompts never match it. It does not create, pause or duplicate this heartbeat, and it does not turn scheduled reporting into a model-free phone push service.
