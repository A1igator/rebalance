...
## UniswapX: existing fillers, no application-operated relayer

The owner's follow-up favors no hosted backend and explicitly suggests UniswapX. This is a stronger existing-service candidate than operating a Calibur relay. Current official documentation lists **UniswapX Dutch V3 on Robinhood 4663**; the July launch announcement also states that Robinhood Stock Tokens are supported through UniswapX. The earlier comparison was incomplete without this route. [Launch](https://blog.uniswap.org/robinhood-chain-is-live), [current chain support](https://developers.uniswap.org/docs/trading/swapping-api/supported-chains).

The local deterministic runner could obtain and independently validate an order, use the selected owner signer, and submit it to the existing filler network. Fillers pay transaction gas, incorporating their costs into the offered exchange terms; this is not a separate USDG paymaster invoice. Signed order constraints must bind tokens, amounts, recipient, chain, reactor, nonce and expiry, and successful on-chain settlement must be verified. Calibur delegation and an application-operated gas wallet are not required for ordinary ERC-20 inputs. [Order submission](https://developers.uniswap.org/docs/api-reference/post_order).

This removes the hosted-backend requirement but does not yet meet the owner's no-key requirement. The documented Trading API quote/order/status flow requires an API key. The public keyless GET orders endpoint is for fillers reading open orders; it is not documented as keyless swapper submission. A user-supplied key could stay in the existing local process rather than a deployed backend or browser bundle, but the owner has not accepted that setup. [Authentication and key handling](https://developers.uniswap.org/docs/trading/swapping-api/start-building/integration-guide), [endpoint distinction](https://developers.uniswap.org/docs/liquidity/uniswapx/filling/faq).

Initial ERC-20 approvals to Permit2 still require on-chain gas for each input token lacking sufficient allowance. Gasless order settlement alone does not solve that bootstrap. Small orders may be unavailable: current docs require a per-chain minimum or at least 0.2% improvement over the AMM alternative, otherwise QuoteAmountTooLowError. No numeric Robinhood threshold or live stock-pair quote was established in this check. No API credential, approval, signature or order was created, and no execution code was changed.
