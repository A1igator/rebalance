...
## Final requested UI behavior

The human chose the minimal supported result: “just disable privy then on the + button after one is done mentioning the limit”, followed by “mentioning it on hover”. In the New portfolio dialog, disable the Privy choice when the loaded registry already contains a Privy portfolio and give it hover text explaining the current Agent Sandbox one-Ethereum-wallet limit. Keep existing portfolio cards usable, retain Local key/Ledger choices, and do not add a dashboard path or persistent warning outside the choice. Validate this against existing and changing portfolio snapshots and preserve first-wallet onboarding.
