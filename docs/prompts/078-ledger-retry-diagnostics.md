...
Before the next UI edit, the owner asked to replace the separate Retry button with a simple refresh icon beside the error text or the retry word in its hint. Keep the same guarded action, add a clear accessible label/tooltip and pending state, avoid repeated Retry wording, and preserve the ring layout. They also reported enabling blind signing on the physical device for this test; no application fallback change has been requested or made.
