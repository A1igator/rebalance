...
During the current migration, `notifications test` provides a labelled nonfinancial end-to-end check. **After that test actually arrives in this conversation**, acknowledge it and remove the matching old five-minute Rebalance heartbeat through the native automation tool. A scheduled heartbeat reading a locally stored test is not native delivery: leave that test unacknowledged and stay quiet until its own native notification prompt arrives. Do not remove the fallback based only on queue acceptance or fixtures, and do not install new periodic model checks. Native Remote/app settings still determine phone behavior; neither acknowledgement nor transport acceptance proves a phone push.
