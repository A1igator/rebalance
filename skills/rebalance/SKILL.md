...
1. A user-reviewed Codex `UserPromptSubmit` hook can handle the exact bare `$rebalance` directly through [`scripts/rebalance-hook.mjs`](../../scripts/rebalance-hook.mjs). It installs missing locked dependencies and calls `launch` with the session/turn request identity. If its result is already in the conversation, report that result; do not call launch/start again. The hook ignores scoped commands, quotations, other events and Plan mode. It never processes a natural-language launch request. See [hook setup and limits](../../docs/LAUNCH.md).
