...
import { delegationFor, isDelegatedExecution, type DelegatedExecution } from './delegation.js';
...
export type DelegatedState = 'undelegated' | DelegatedExecution;
...
export function validateDelegatedTransaction(config: Config, tx: ChainTransaction): void {
  const metadata = isDelegatedExecution(config.execution) ? tx[config.execution] : undefined;
  if (!isDelegatedExecution(config.execution) || config.mode !== 'ledger' || config.chainId !== 4663 ||
      ...
      tx.plan.trades.length !== tx.swapCount || tx.approvalCount !== 0 || !metadata || !Number.isInteger(metadata.approvalCount) ||
      metadata.approvalCount < 0 || metadata.approvalCount > 4 || (tx.calibur !== undefined && tx.simple7702 !== undefined)) {
    throw new Error('Delegated execution requires an explicit Ledger wallet and an atomic rebalance');
  }
  const calls = delegationFor(config.execution).decodeCalls(tx.data);
  if (calls.length !== metadata.approvalCount + 1) throw new Error('Invalid delegated batch');
  ...
  for (const call of calls.slice(0, -1)) {
    ...
    if (!token || !Object.hasOwn(config.targets, token.id) || approved.has(token.id) || call.value !== 0n) {
      throw new Error('Invalid delegated approval target');
    }
    ...
    if (approval.functionName !== 'approve' || getAddress(approval.args[0]) !== getAddress(ROUTER) || approval.args[1] <= 0n) {
      throw new Error('Invalid delegated router approval');
    }
    ...
    if (input === undefined || input !== approval.args[1]) throw new Error('Delegated approval differs from the prepared input');
    approved.add(token.id);
  }
  const routerCall = calls.at(-1)!;
  if (getAddress(routerCall.to) !== getAddress(ROUTER) || routerCall.value !== 0n) throw new Error('Invalid delegated swap target');
  ...
  if (router.functionName !== 'multicall' || router.args[1].length !== tx.swapCount ||
      tx.expiresAt === undefined || router.args[0] < tx.expiresAt) throw new Error('Invalid delegated swap batch');
  ...
  for (const [index, data] of router.args[1].entries()) {
    ...
    if (swap.functionName !== 'exactInputSingle') throw new Error('Invalid delegated inner swap');
    ...
    if (!sell || !buy || sell.id === buy.id || !Object.hasOwn(config.targets, sell.id) || !Object.hasOwn(config.targets, buy.id) ||
        ...
        getAddress(params.recipient) !== getAddress(config.wallet)) throw new Error('Delegated swap differs from its prepared plan');
    const stock = sell.id === 'USDG' ? buy.id : sell.id;
    if (usedStocks.has(stock) || (purchasing && sell.id !== 'USDG')) throw new Error('Invalid delegated trade order');
...
export async function readDelegatedState(chain: Chain, wallet: Address, execution: DelegatedExecution): Promise<DelegatedState> {
  const descriptor = delegationFor(execution);
  const rpc = chain.publicClient;
  if (await rpc.getChainId() !== 4663) throw new Error('Delegated execution requires Robinhood mainnet');
  const blockNumber = await rpc.getBlockNumber({ cacheTime: 0 });
  const [implementation, account] = await Promise.all([
    rpc.getCode({ address: descriptor.address, blockNumber }),
    rpc.getCode({ address: wallet, blockNumber }),
  ]);
  descriptor.assertDeployment(implementation);
  return descriptor.inspectAccountCode(account);
}

/** Full atomic call simulation before device authorization. No signed authorization leaves the host. */
export async function estimateDelegatedGas(config: Config, chain: Chain, tx: ChainTransaction, state: DelegatedState): Promise<bigint> {
  validateDelegatedTransaction(config, tx);
  if (!isDelegatedExecution(config.execution) || (state !== 'undelegated' && state !== config.execution)) throw new Error('Delegation differs from the explicit execution mode');
  const estimate = await chain.publicClient.estimateGas({ account: config.wallet, to: tx.to, data: tx.data, value: 0n,
    ...(state === 'undelegated' ? { stateOverride: [{ address: config.wallet, code: delegationFor(config.execution).delegationCode }] } : {}),
  });
  if (typeof estimate !== 'bigint' || estimate <= 0n) throw new Error('Invalid delegated gas estimate');
