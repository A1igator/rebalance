...
import { buildCaliburSetupTransaction, CALIBUR_ADDRESS, CALIBUR_DELEGATION_CODE } from './calibur.js';
...
export type CaliburSetupResult = {
  app: 'Rebalance'; operation: 'calibur-setup'; wallet: string; chainId: 4663;
...
export async function setupCalibur(options: { signal?: AbortSignal; expectedStop?: string } = {}, dependencies: CaliburSetupDependencies = {}): Promise<CaliburSetupResult> {
  options = { ...options };
  ...
  const stagePath = resolve(DATA, 'calibur-setup-status.json'), requestId = randomUUID();
  try {
    const configGeneration = await controlGeneration(CONFIG_PATH);
    const config = structuredClone(await loadConfig());
    if (!config || config.mode !== 'ledger' || config.execution !== 'calibur') throw new Error('Calibur setup requires the saved Ledger Calibur configuration');
    ...
    const base = { app: 'Rebalance', operation: 'calibur-setup', wallet: config.wallet, chainId: 4663 } as const;
    ...
    if (existing) {
      ...
      if (existing.kind !== 'calibur-setup') return { ...base, outcome: 'existing-transaction', pendingKind: existing.kind, hash: existing.hash, status };
    ...
    const state = await readCaliburState(chain, config.wallet);
    await localReady();
    if (state === 'calibur') return { ...base, outcome: 'already-enabled' };
    ...
    const tx = buildCaliburSetupTransaction(config.wallet);
    let gas: bigint;
    try {
      const estimate = await rpc.estimateGas({ account: config.wallet, ...tx,
        stateOverride: [{ address: config.wallet, code: CALIBUR_DELEGATION_CODE }] });
    ...
    const feeInput: FeeTargetInput | undefined = config.rebalanceFeeTargetUsdE8 === undefined ? undefined : {
      targetUsdE8: config.rebalanceFeeTargetUsdE8, kind: 'calibur-setup', swaps: 0, swapsInCurrentTransaction: 0,
    ...
    const current = async () => {
      ...
      const [pendingNonce, latestNonce, currentState, balance] = await Promise.all([
        rpc.getTransactionCount({ address: config.wallet, blockTag: 'pending' }),
        rpc.getTransactionCount({ address: config.wallet, blockTag: 'latest' }),
        readCaliburState(chain, config.wallet), rpc.getBalance({ address: config.wallet, blockTag: 'pending' }),
      ]);
    ...
    authorization = structuredClone(await account.signDelegationAuthorization({ chainId: 4663, address: CALIBUR_ADDRESS, nonce: nonce + 1 }));
    await current();
    try {
      if (authorization.chainId !== 4663 || authorization.address.toLowerCase() !== CALIBUR_ADDRESS.toLowerCase() || authorization.nonce !== nonce + 1 ||
    ...
    const pending: PendingTransaction = { chainId: 4663, wallet: config.wallet, hash, nonce, kind: 'calibur-setup',
...
export async function caliburSetupStatus(dependencies: Pick<CaliburSetupDependencies, 'chain'> = {}): Promise<CaliburSetupResult> {
  ...
  const pendingSummary = (pending: PendingTransaction): CaliburSetupResult => {
    ...
    return { ...base, outcome: pending.kind === 'calibur-setup' ? 'confirming' : 'existing-transaction', pendingKind: pending.kind, hash: pending.hash };
  };
  const activeStage = async (): Promise<CaliburSetupResult | null> => {
    const [run, stage] = await Promise.all([
      ...
      readJson<{ version?: number; wallet?: string; chainId?: number; runToken?: string; stage?: string; updatedAt?: string }>(resolve(DATA, 'calibur-setup-status.json')),
    ]);
  ...
  if (!pending) {
    ...
    if (!pending) {
      ...
      return { ...base, outcome: state === 'calibur' ? 'already-enabled' : 'needed' };
    }
  }
  ...
  try {
    ...
    pending = await readJson<PendingTransaction>(PENDING_PATH);
    if (!pending) {
      if (observedPending.kind !== 'calibur-setup') return { ...base, outcome: 'existing-transaction', pendingKind: observedPending.kind, hash: observedPending.hash, status: 'unresolved' };
      const state = await readCaliburState(chain, config.wallet);
      if (JSON.stringify(await loadConfig()) !== JSON.stringify(config)) throw new Error('Configuration changed while reading Calibur status');
      const latest = await readJson<PendingTransaction>(PENDING_PATH);
      if (latest) return pendingSummary(latest);
      return { ...base, outcome: state === 'calibur' ? 'already-enabled' : 'unknown' };
    }
    validatePending(pending, config);
    const observed = await reconcile(config, chain), status = observed.operation?.status ?? 'unresolved';
    if (pending.kind !== 'calibur-setup') return { ...base, outcome: 'existing-transaction', pendingKind: pending.kind, hash: pending.hash, status };
