...
export type RecoveryRecord = {
  ...
  priorStop: string; stop: Stop; cancellation?: Cancellation;
  resolution?: 'original-confirmed' | 'original-reverted' | 'cancelled'; resolvedAt?: string;
  operation?: Operation; resumeAttemptedAt?: string;
};
...
function validateRecord(record: RecoveryRecord, config: Config): void {
  if (!record || record.version !== 1 || typeof record.originallyArmed !== 'boolean' ||
      !/^(none|[a-f0-9]{64})$/.test(record.priorStop) || typeof record.stop?.requestId !== 'string' ||
  ...
  try {
    ...
    const receipt = async (hash: Hex, cancellation: boolean) => {
      ...
      if (cancellation && (found.to?.toLowerCase() !== config.wallet.toLowerCase() || tx.to?.toLowerCase() !== config.wallet.toLowerCase() ||
          tx.value !== 0n || tx.input !== '0x' || found.status !== 'success')) {
        throw new RecoveryError('Cancellation is not a successful zero-value empty-input self-transfer.');
      }
    ...
    const assessment = async (): Promise<Assessment> => {
      const first = await receipt(original.hash as Hex, false);
      if (first) return first.confirmed ? { outcome: first.operation.status === 'confirmed' ? 'original-confirmed' as const : 'original-reverted' as const, operation: first.operation } : { outcome: 'confirming' as const };
      if (record?.cancellation) {
        const cancelled = await receipt(record.cancellation.hash, true);
        if (cancelled) return cancelled.confirmed ? { outcome: 'cancelled' as const, operation: cancelled.operation } : { outcome: 'confirming' as const };
        return { outcome: 'pending' as const };
      }
      return { outcome: original.status === 'broadcast' ? 'pending' as const : 'cancellation-needed' as const };
    };
    ...
    if (assessed.outcome === 'cancellation-needed') {
      ...
      if (config.mode !== 'private-key') throw new RecoveryError('Cancellation currently requires the selected raw-key signer; no fallback was used.');
      const tx = await getTransaction(original.hash as Hex);
      ...
      if (tx && (tx.from.toLowerCase() !== config.wallet.toLowerCase() || tx.nonce !== original.nonce || tx.blockNumber !== null)) throw new RecoveryError('Original transaction identity changed or it is already mined; wait for its receipt.');
      ...
      const fee = (tx?.gasPrice && tx.gasPrice > marketFee ? tx.gasPrice : marketFee) * 2n;
      const gas = (await rpc.estimateGas({ account: config.wallet, to: config.wallet, data: '0x', value: 0n }) * 120n + 99n) / 100n;
      if (fee <= 0n || gas <= 0n || await rpc.getBalance({ address: config.wallet, blockTag: 'pending' }) < fee * gas) throw new RecoveryError('Insufficient native gas balance for explicit cancellation.');
      await ensureStop();
      ...
      // Reconcile again after preparation to catch the original winning the race.
      assessed = await assessment();
      if (assessed.outcome === 'cancellation-needed') {
        await ensureStop();
        const hash = keccak256(serialized);
        record.cancellation = { hash, gas: gas.toString(), gasPrice: fee.toString(), status: 'prepared' };
        result.cancellationHash = hash;
        await atomicWriteJson(path('recovery.json'), record);
        try { await ensureStop(); }
    ...
    record.resolution = assessed.outcome;
    record.resolvedAt = new Date().toISOString(); record.operation = assessed.operation;
    await atomicWriteJson(path('recovery.json'), record);
    ...
    if (pending) {
      ...
      if (transactionIdentity(pending) !== transactionIdentity(original)) throw new RecoveryError('Pending identity changed before recovery completion.');
