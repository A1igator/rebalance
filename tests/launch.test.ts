...
const usdGasPayment = { provider: 'alchemy', token: 'USDG', policyId: '11111111-2222-3333-4444-555555555555',
  paymaster: '0x0000000000000000000000000000000000000011' } as const;

test('USDG gas-payment launch permits a zero-native-ETH wallet after a successful runtime check', async t => {
  const f = await fixture(t);
  f.current.config!.gasPayment = usdGasPayment; f.current.nativeBalance = 0n;
  const launched = await launch({}, f.deps);
  assert.equal(launched.outcome, 'armed');
  assert.equal(launched.status?.nativeBalance, 0n);
  assert.deepEqual(launched.status?.config?.gasPayment, usdGasPayment);
  assert.equal(count(f.calls, 'check'), 1); assert.equal(count(f.calls, 'start'), 1);
  assert.equal(count(f.calls, 'configure'), 0);
});

test('USDG provider or funding preflight failures prevent launch without native fallback', async t => {
  const f = await fixture(t), command = f.deps.command;
  f.current.config!.gasPayment = usdGasPayment; f.current.nativeBalance = 0n;
  f.deps.command = async args => {
    if (args[0] !== 'check') return command(args);
    f.calls.push(args); f.current.error = 'USDG provider setup or fee balance is unavailable';
    return { ok: false, value: structuredClone(f.current) };
  };
  const launched = await launch({}, f.deps);
  assert.equal(launched.outcome, 'blocked'); assert.equal(count(f.calls, 'start'), 0);
  assert.equal(count(f.calls, 'configure'), 0);
  assert.deepEqual(f.current.config!.gasPayment, usdGasPayment);
});

test('changing payment transport or policy while chart preflight runs invalidates its readiness', async t => {
  for (const change of ['enable', 'disable', 'policy'] as const) {
    const f = await fixture(t);
    if (change !== 'enable') f.current.config!.gasPayment = usdGasPayment;
    f.deps.chartStatus = async () => {
      const checked = structuredClone(f.current);
      if (change === 'disable') delete f.current.config!.gasPayment;
      else f.current.config!.gasPayment = { ...usdGasPayment,
        policyId: change === 'policy' ? '22222222-2222-3333-4444-555555555555' : usdGasPayment.policyId };
      return { state: 'response', value: checked };
    };
    const launched = await launch({}, f.deps);
    assert.equal(launched.outcome, 'blocked', change); assert.equal(count(f.calls, 'start'), 0, change);
    assert.match(launched.messages.join(' '), /Configuration changed during launch/, change);
  }
});
