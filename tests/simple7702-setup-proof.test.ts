...
import { buildSimple7702SelfTransaction, buildSimple7702SetupTransaction, SIMPLE7702_ABI, SIMPLE7702_ADDRESS, SIMPLE7702_DELEGATION_CODE } from '../src/simple7702.js';
import { buildCaliburSetupTransaction, CALIBUR_ADDRESS, CALIBUR_DELEGATION_CODE } from '../src/calibur.js';
...
const data = await mkdtemp(join(tmpdir(), 'rebalance-simple7702-proof-'));
...
const { verifyDelegatedSetupReceipt } = await import('../src/calibur-setup-proof.js');
...
const config = validateConfig({ version: 1, chainId: 4663, mode: 'ledger', execution: 'simple7702', wallet,
...
const pending: PendingTransaction = { chainId: 4663, wallet, hash, kind: 'simple7702-setup', nonce: 7, status: 'broadcast', createdAt: '2026-09-13T06:00:00.000Z' };
...
function fixture(status: 'success' | 'reverted' = 'success') {
  ...
  const tx = { type: 'eip7702', hash, from: wallet, to: wallet, value: 0n, input: buildSimple7702SetupTransaction(wallet).data,
    nonce: 7, chainId: 4663, blockNumber: 100n, blockHash,
    authorizationList: [{ chainId: 4663, nonce: 8, address: SIMPLE7702_ADDRESS as Address }] };
  const state = { code: SIMPLE7702_DELEGATION_CODE as Hex | undefined, implementation: evidence.runtimeBytecode as Hex,
  ...
  const rpc = {
    ...
    async getCode(input: { address: Address; blockNumber: bigint }) {
      ...
      if (input.address.toLowerCase() === SIMPLE7702_ADDRESS.toLowerCase()) return state.implementation;
...
const mismatches: [string, (f: Fixture) => void][] = [
  ...
  ['nonempty batch', f => { f.tx.input = buildSimple7702SelfTransaction(wallet, [{ to: other, value: 0n, data: '0x12345678' }]).data; }],
  ['legacy Calibur empty batch', f => { f.tx.input = buildCaliburSetupTransaction(wallet).data; }],
  ['legacy Calibur delegation', f => { f.state.code = CALIBUR_DELEGATION_CODE; }],
  ['legacy Calibur authorization', f => { f.tx.authorizationList[0]!.address = CALIBUR_ADDRESS; }],
...
test('verified empty type4 setup with canonical delegation clears the common receipt barrier without recording a swap', async () => {
  ...
  await verifyDelegatedSetupReceipt(config, f.chain, pending, f.receipt as TransactionReceipt);
  assert.equal(await readJson(PENDING_PATH) !== null, true, 'proof alone is read-only');
  const result = await reconcile(config, f.chain);
  assert.equal(result.blocked, false); assert.equal(result.operation?.kind, 'simple7702-setup'); assert.equal(result.operation?.status, 'confirmed');
  assert.equal(await readJson(PENDING_PATH), null);
  assert.equal((await readJson<{ kind: string }>(LAST_TRANSACTION_PATH))?.kind, 'simple7702-setup');
...
test('receipt recovery clears only proven setup, including a reverted execution, and never changes cycle timing', async () => {
  for (const status of ['success', 'reverted'] as const) {
    ...
    try {
      ...
      assert.equal(result?.blocked, false); assert.equal(result?.operation?.kind, 'simple7702-setup');
