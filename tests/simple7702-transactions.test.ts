...
import { CALIBUR_ADDRESS, CALIBUR_DELEGATION_CODE, buildCaliburSelfTransaction } from '../src/calibur.js';
import { delegationFor } from '../src/delegation.js';
import { SIMPLE7702_ABI, SIMPLE7702_ADDRESS, SIMPLE7702_DELEGATION_CODE, buildSimple7702SelfTransaction } from '../src/simple7702.js';
...
const data = await mkdtemp(join(tmpdir(), 'rebalance-simple7702-dispatch-'));
...
assert.match(implementationCode, /^0x[0-9a-f]+$/i, 'Committed public Simple7702 runtime fixture is required');
...
function transaction(): ChainTransaction {
  ...
  return { ...buildSimple7702SelfTransaction(wallet, calls), kind: 'swap', expiresAt, swapCount: 4, approvalCount: 0,
    plan: { trades, reason: 'public offline combined fixture' }, simple7702: { approvalCount: 2 } };
}
function fixture(delegated = false) {
  const state = { nonce: 7, accountCode: delegated ? SIMPLE7702_DELEGATION_CODE as Hex | undefined : undefined,
  ...
  const rpc = {
    ...
    getCode: async ({ address }: { address: Address }) => address.toLowerCase() === SIMPLE7702_ADDRESS.toLowerCase() ? state.implementation : state.accountCode,
...
test('first Simple7702 use simulates full call before nonce+1 authorization and sends one type4 transaction', async () => {
  ...
  const batch = { calls: decodeFunctionData({ abi: SIMPLE7702_ABI, data: parsed.data! }).args[0].map(call => ({ ...call, to: call.target })) };
...
test('Stop, settings, nonce or account changes after authorization prevent outer signing and broadcast', async () => {
  for (const change of ['stop', 'config', 'nonce', 'account', 'implementation']) {
    ...
    f.afterAuthorization(async () => {
      ...
      if (change === 'account') f.state.accountCode = SIMPLE7702_DELEGATION_CODE;
...
test('unknown Simple7702 send retains only one outer hash and prevents a second authorization or send', async () => {
  const f = fixture();
...
test('a reverted enrollment keeps its receipt barrier; explicit receipt recovery retries with the existing delegation and nonce N+2', async () => {
  ...
  f.state.accountCode = SIMPLE7702_DELEGATION_CODE; f.state.nonce = 9;
  ...
  assert.equal(f.state.accountCode, SIMPLE7702_DELEGATION_CODE); assert.equal(f.state.nonce, 9);
...
test('Simple7702 cannot accept legacy Calibur metadata, calldata, account designation or authorization', async () => {
  for (const mismatch of ['metadata', 'both-markers', 'calldata', 'designation', 'authorization']) {
    const f = fixture(), tx = transaction();
    if (mismatch === 'metadata') { tx.calibur = tx.simple7702; delete tx.simple7702; }
    if (mismatch === 'both-markers') tx.calibur = { approvalCount: 2 };
    if (mismatch === 'calldata') tx.data = buildCaliburSelfTransaction(wallet, delegationFor('simple7702').decodeCalls(tx.data)).data;
    if (mismatch === 'designation') f.state.accountCode = CALIBUR_DELEGATION_CODE;
    const signer = mismatch !== 'authorization' ? f.signer : async () => ({ ...(await f.signer()),
      async signDelegationAuthorization(request: CaliburAuthorizationRequest) {
        f.authorizations.push(request);
        return account.signAuthorization({ ...request, address: CALIBUR_ADDRESS });
      },
    });
    await assert.rejects(dispatch(configuration(), f.chain, tx, signer, f.ledger));
    assert.equal(f.authorizations.length, mismatch === 'authorization' ? 1 : 0);
    assert.equal(f.signed.length, 0); assert.equal(f.sent.length, 0);
    assert.equal(await readJson(PENDING_PATH), null);
  }
});
